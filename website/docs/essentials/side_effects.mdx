...
Instead, providers must explicitly expose a way to modify their state.
...
Let's start with what we already know by this point: A plain and simple _GET_ request.
...
This class handles exposing ways to modify the state of the provider.  
...
would have no way of knowing that state has changed.
:::
...
Notifiers handle exposing ways to modify the state of the provider.  
Public methods on this class are accessible to consumers using `ref.read(yourProvider.notifier).yourMethod()`.

:::note
Notifiers should not have public properties besides the built-in `state`, as the UI
would have no way of knowing that the state has changed.
:::
...
There are a few ways to do this with their pros and cons.
...
- The UI will have the most up-to-date state possible.
  If another user added a todo, we would see it too.
...
Another way is to have our provider re-execute the _GET_ request.  
...
One other way is to update the local cache manually.  
This would involve trying to mimic the backends’ behavior.
...
:::info
This example uses immutable state. This is not required but it is recommended.
...
- The local cache may not match the server's state.
  If another user added a todo, we would not see it.
- This approach may be more complex to implement and effectively
  duplicate the backends’ logic.

:::

## Going further: Showing a spinner & error handling

Here is what we have seen so far, we have a button which makes a _POST_ request
when pressed; and when the request is done, the UI updates to reflect the
changes.  
But at the moment, there is no indication that the request is being
performed, nor any information if it failed.

One way to do this is to store the Future returned by `addTodo`
...
an operation is pending. And if it failed, it will render the button as red:
